...
if nargin<2,    precision='single';   end
[b,m,q]=nufftparams(precision);

if ndims(beta) ~= 3,    error('beta must be a cubed volume');  end
...
low_idx  = -ceil((n-1)/2);        high_idx = floor((n-1)/2);
idx      = low_idx:high_idx;

E  = exp(b.*(2.*pi.*idx./(m*n)).^2);    E  = E(:);
EE = E*E.';   E3 = zeros(n,n,n);
for k=1:n,    E3(:,:,k)=EE.*E(k);    end
