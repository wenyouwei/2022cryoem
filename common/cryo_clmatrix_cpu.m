...
if mod(T,2)~=0,    error('n_theta must be even');   end
...
n_theta = size(pf,2);      n_proj  = size(pf,3);

%% Check input parameters and set debug flags.
NK  =  n_proj;
if (nargin<2) || (NK==-1), NK = n_proj; end % Number of common-line pairs to compute for each projection
