...
n=64;               % Size of the projections. Can be even or odd.
...
p1=cryo_project(vol,q);   % Project the volume in the z direction.
p2=sum(vol ,3);      % Sum the volume in the z direction. 
err=norm(p1(:)-p2(:))/norm(p1(:));
disp(err); % Error should be close to machine precision.

maskR = 31; 
RotMatrix = inv(q_to_rot(q));  RotMatrix = RotMatrix(:);
tic; p3 = OpForwardProj(vol, RotMatrix, maskR); toc
err=norm(p1(:)-p3(:))/norm(p1(:));
disp(err); % Error should be close to machine precision.
...
clear;
voldef='C1_params'; % Can be 'one_ball' for a simpler phantom.
q = qrand(1);       % Some random rotation.
...
p1=cryo_project(vol,q)*dz; % Project the volume and multiply by dz to 
                           % convert the discrete projection (summation
                           % along the direction specified by q) into an
                           % integral. 
p2=cryo_project_gaussian(voldef,n,rmax,q); % Compute analytic projection.
...
% For a given rotation q, generate the points in which to sample the
...
[p1hat,nf]=cryo_gaussian_ft_slice(voldef,n,rmax,q);
...
p1=cryo_project(vol,q)*dz; % =cryo_project_gaussian(voldef,n,rmax,q);
%p2=cryo_project_gaussian(voldef,n,rmax,q);
...
p2hat=cryo_gaussian_ft_slice(voldef,n,rmax,q);
err=norm(p1hat(:)-p2hat(:))/norm(p1hat(:));
disp(err); % Error should be close to machine precision.
