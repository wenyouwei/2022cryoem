...
tic;
[clean_projections_2, noisy_projections_2, shifts, q] =  cryo_gen_projections(n,K,SNR,[],[],ref_shifts,ref_q,'single');
toc

for k=1:K,    inv_rot_matrices(:,:,k)   = q_to_rot(q(:,k)); end
RotMatrix    = reshape(inv_rot_matrices, 9, K);
load cleanrib; 
projs = CryoForWardProj(volref,RotMatrix,n,'single');
