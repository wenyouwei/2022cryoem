function [ v, v_b, kernel ,err, iter, flag] = recon3d_firm( projections,...
...
if isempty(max_it), max_it=1000; end
...
if exist('fprecomp','var')
    [v_b,kernel] = precomp( projs_fourier,inv_rot_matrices,fprecomp);
else
    [v_b,kernel] = precomp( projs_fourier,inv_rot_matrices);
end
% toc;
...
% %
% lambda = 1e-0; 
% H = fftnCryoEMkernel(kernel); 
% % v2 = pcg(@(x)CryoEMkerneltimesx(H,x), v_b(:),tol, max_it,[],[],x0(:)); 
% R = 0 * H; 
% R(1,1,1) = -3; R(1,1,2) = 1; R(1,2,1) = 1; R(2,1,1) = 1; 
% R = fftn(R).*fftn(R); 
% H = H + lambda * R; 
% tic; 
% AtA = @(x)(CryoEMkerneltimesx(H,x) ); 
% v2 = pcg(@(x)AtA(x), v_b(:),tol, max_it,[],[],x0(:)); 
% toc
